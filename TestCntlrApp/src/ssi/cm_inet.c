...
/********************************************************************20**
        ...
        Sid:      cm_inet.c@@/main/64 - Tue Dec 10 11:42:15 2013
...
/*cm_inet_c_001.main_63: Poll related changes*/
/*
  *
  *      Fun:   cmInetPollRetrieveIdx
  *
  *      Desc:  This is useful to retrieve the index of the
  *             fd set which is compared against the event passed.  
  *
  *      Ret:   Index of the array.
  *
  *
  *      Notes: We make sure that for the revents set to
  *             POLLPRI or (POLLERR | POLLIN) 1.e 9, we read the
  *             data fromt he fd set.
  *
  *      File:  cm_inet.c
  *
  */
#ifdef ANSI
PUBLIC S16 cmInetPollRetrieveIdx
(
  CmInetPollFd  *pollFdArr,             /* poll FD Array */
  U16            eventMask,             /* Event Mask */
  S16            startIdx
)
#else
PUBLIC S16 cmInetPollRetrieveIdx(pollFdArr, eventMask, startIdx)
    CmInetPollFd  *pollFdArr;            /* poll FD Array */
    U16            eventMask;            /* Event Mask */
    S16            startIdx;
#endif
...
/* error check on parameters */
    ...
    for (idx = startIdx; idx < CM_INET_POLL_MAXFDSUPP; idx++)
    {
       if(eventMask ==POLLIN)
       {
          if((pollFdArr[idx].revents==POLLPRI)||(pollFdArr[idx].revents==eventMask)
                ||(pollFdArr[idx].revents==(POLLERR|POLLIN)))
          {
              break;
          }
       ...
       {
          if (pollFdArr[idx].revents == eventMask)
          {
             break;
          }
       }
...
/*
 *
 *      Fun:   cmInetGetPollFdIdx
 *
 *      Desc:
 *
 *      Ret:   Fd index from the Poll array.
 *
 *
 *      Notes:  This is useful to retrieve the Fd index on
 *              basis of Fd used in Poll Array.
 *
 *
 *      File:  cm_inet.c
 *
...
#ifdef ANSI
PUBLIC S16 cmInetGetPollFdIdx
(
  CmInetPollFd  *pollFdArr,             /* poll FD Array */
  CmInetFdType   fd                     /* socket descriptor */
)
#else
PUBLIC S16 cmInetGetPollFdIdx(pollFdArr, fd)
   CmInetPollFd  *pollFdArr;          /* poll FD Array */
   CmInetFdType   fd;                  /* socket descriptor */
#endif
{
...
/********************************************************************30**
         ...
         End of file:     cm_inet.c@@/main/64 - Tue Dec 10 11:42:15 2013
...
/main/63     ---      cm_inet_c_001.main_62 akaranth 1.Modified for warning fix
/main/64     ---      cm_inet_c_001.main_63 mladdha  1. Added poll related changes.
*********************************************************************91*/
