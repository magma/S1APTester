 ...
 *          -# Success : ROK
 */
PUBLIC S16 NbEnbHandleInitCtxtSetupRspFailedErabs(NbInitCtxtSetupFailedErabs *icsRspFailedErabs)
{
   NB_LOG_ENTERFN(&nbCb);
   U8 idx;
   ...
   nbCb.initCtxtSetupFailedErabs[(icsRspFailedErabs->ueId) - 1].flag  = icsRspFailedErabs->flag;
   nbCb.initCtxtSetupFailedErabs[(icsRspFailedErabs->ueId) - 1].numFailedErabs  = icsRspFailedErabs->numFailedErabs;
   for (idx = 0;idx < icsRspFailedErabs->numFailedErabs; idx++) {
     nbCb.initCtxtSetupFailedErabs[(icsRspFailedErabs->ueId) - 1].failedErabs[idx]  = icsRspFailedErabs->failedErabs[idx];
   }
   nbCb.initCtxtSetupFailedErabs[(icsRspFailedErabs->ueId) - 1].cause.causeTyp  = NB_CAUSE_TRANSPORT;
   nbCb.initCtxtSetupFailedErabs[(icsRspFailedErabs->ueId) - 1].cause.causeVal  = CAUSE_TRANSPORT_RESOURCE_UNAVAILABLE;
   RETVALUE(ROK);
}
